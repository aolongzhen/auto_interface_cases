...
MEMBER_ID=None#成员id全局变量,方便我们后期去调用
LEAVE_AMOUNT=None#成员LEAVE_AMOUNT全局变量,方便我们后期去调用
#日志类
logger=MyLog()
    ...
    def test_http_request(self,a):#a字典
        ...
        if a['params'].find('${member_id}')!=-1:
            ...
            if param.find('${loan_id}')!=-1:
                ...
                param=param.replace('${loan_id}',str(loan_id))#替换loan_id
        elif a['params'].find('${loan_id}')!=-1:#这里是审核标，投资前的操作
             loan_id=DoMysql().do_mysql("select id from loan where memberid="+str(MEMBER_ID))[0]
             param=a['params'].replace('${loan_id}',str(loan_id))#替换loan_id
             global LEAVE_AMOUNT#投资前的金额
             LEAVE_AMOUNT=DoMysql().do_mysql("select leaveamount from member where id="+str(MEMBER_ID))[0]
        else:
            param=a['params']
        ...
        if a['check_sql']!=None:#是否需要对数据库进行检查？
            if a['check_sql'].find('${after_invest_money}')!=-1:
               query_sql=a['check_sql'].replace('${after_invest_money}',str(LEAVE_AMOUNT-100))
               #这里投资金额都是写死的100 所以这里直接减去100，否则也要利用全局变量来存储值
            else:
        ...
        if a['expect_result'].find('${member_id}')!=-1 and a['expect_result'].find('${regtime}')!=-1:
            ...
            global MEMBER_ID#注册成功之后  去更新MEMBER_ID这个全局变量
            MEMBER_ID=member_id#替换全局变量，所以我们执行的顺序不能变~一旦改变用例就是去了依赖性
